  ...
  return (
    <div id="grammar-reminder-modal-overlay" className="fixed inset-0 z-50 bg-black/40 backdrop-blur-md flex items-center justify-center p-3 sm:p-4">
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden border border-black/[0.08]">
        ...
        <div className="p-4 sm:p-5 border-b border-black/[0.05] flex items-center justify-between gap-3 bg-white">
          <div>
            <div className="flex items-center gap-2">
              <span className="inline-block px-2.5 py-0.5 rounded-full text-sm font-semibold bg-black/[0.04] text-[#202124]">
                A1 – M22 Reference
              </span>
        ...
        {/* Filter Bar */}
        <div className="p-3 border-b border-black/[0.05] bg-[#F8F9FA] flex flex-col sm:flex-row items-center gap-2.5">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-sm sm:text-base bg-white border border-black/[0.1] rounded-full focus:outline-none focus:ring-2 focus:ring-[#4285F4] text-[#202124]"
            />
          </div>
          <select
            value={selectedCategory}
            onChange={(e) => setSelectedCategory(e.target.value)}
            className="w-full sm:w-auto px-3 py-1.5 text-sm bg-white border border-black/[0.1] rounded-full text-[#202124] focus:outline-none focus:ring-2 focus:ring-[#4285F4]"
          >
              ...
              <div key={item.code} className="pt-3.5 first:pt-0 space-y-2.5">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2.5">
                    <span className="w-7 h-7 rounded-full bg-[#202124] text-white font-semibold text-sm flex items-center justify-center">
                      {item.code}
                    </span>
                    <div>
                ...
                <div className="flex items-center justify-between p-3 bg-[#F8F9FA] rounded-xl text-sm text-[#202124] border border-black/[0.04]">
                  <span className="font-normal italic">Example: {item.example}</span>
                  <button
                    onClick={() => speakText(item.example)}
                    title="Listen to audio"
                    className="p-1 rounded-full text-[#333336] hover:text-[#4285F4] hover:bg-black/[0.05] transition-colors cursor-pointer"
                  >
        ...
        {/* Modal Footer */}
        <div className="p-3.5 bg-[#F8F9FA] border-t border-black/[0.05] flex justify-end">
          <button
            onClick={onClose}
